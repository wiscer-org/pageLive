...
const geminiPageAdapter = async () => {
    ...
    // Only attach to `pageLive.page` when have tried to parse the info.
    const EMPTY_CHAT: Chat = {
        // const EMPTY_CHAT: typeof pageLive.= {
    ...
    async function getChatActionsMenuElement(): Promise<HTMLElement | null> {
        // Only use the top actions menu, for both large and medium / small screen
        // This is for the large width screen.
        let chatActionsMenu = document.querySelector('conversation-actions-icon') as HTMLElement | null;

        if (chatActionsMenu === null) {
            const msg = "Failed to find chat actions menu.";
    ...
    async function getDeleteButton(): Promise<HTMLElement | null> {
        ...
        if (deleteButton === null) {
            const msg = "Failed to find the button to delete chat.";
            pl.utils.prodWarn(`[PageLive][Gemini] ${msg}`);
            pl.speak(msg);
        }
    ...
    async function currentChatDelete() {
        ...
        if (isUnsavedChat) {
            pl.toast('This is a new chat. Nothing to delete.');
            return;
        }
        ...
        if (!menuButton) {
            pl.speak('Can not find menu button to delete chat.');
            return;
        }
    ...
    async function getActiveChatMenuButton(): Promise<HTMLElement | null> {
        ...
        let chatMenuButton = document.querySelector('[data-test-id="conversation-actions-menu-icon-button"] button') as HTMLElement | null;
        ...
        while (true) {
            ...
            await new Promise(async (resolve) => {
                ...
                setTimeout(() => {
                    ...
                    if (!loadingStarted) {
                        resolve(null);
                        pl.speak('Sidebar chat list is fully retrieved.')
...
class ContentRevealer {
    ...
    async init(chatContainer: HTMLElement | null) {
        if (!chatContainer) {
            this.pl.utils.prodWarn('[ContentRevealer] received chat container is null - j3d');
            return;
        }
...
class ContentMapper {
        ...
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting) {
                ...
                if (this.chatUnits.length > 0) {
                    ...
                    this.pl.utils.devLog('scrolling to the first chat unit');
                }
            }
        });
    }, {
    ...
    async constructDialog() {
        ...
        this.pl.container.appendChild(this.dialog);
    ...
    async announce(options: { msg: string, omitPreannounce?: boolean }) {
        if (!this.dialog.open) this.pl.announce(options);
        else {
    ...
    async generateChatUnits(shouldRender = true, retry = 15) {
        ...
        if (retry < 1) {
            this.pl.utils.prodWarn(`Run out of retry - 209`);
            return;
        }
        ...
        for (let i = 0; i < promptResponseElements.length; i++) {
            ...
            if (promptElement === null) {
                this.pl.utils.prodWarn("[ContentMap] Failed to find prompt element - 429");
                ...
                chatUnits.push({
                    ...
                    shortContent: this.pl.utils.shortenText(promptElement.textContent),
                });
                // Set tabindex
                promptElement.setAttribute("tabindex", "-1");
                // Set 'element ref attribute' if not available to the element
                if (!promptElement.getAttribute(ContentMapper.EL_REF_ATTR)) promptElement.setAttribute(ContentMapper.EL_REF_ATTR, this.pl.utils.uniqueNumber() + "");
            }
            ...
            if (responseElement === null) {
                this.pl.utils.prodWarn(`[ContentMap] Failed to find element "${RESPONSE_CONTENT_ELEMENT_NAME}" inside below element - 842`);
                console.error(el);
                this.pl.utils.prodWarn(`element connected: ${el.isConnected}`);
                ...
                chatUnits.push({
                    ...
                    shortContent: this.pl.utils.shortenText(responseElement.textContent, 30),
                });
                // Set tabindex
                responseElement.setAttribute("tabindex", "-1");
                // Set 'element ref attribute' if not available to the element
                if (!responseElement.getAttribute(ContentMapper.EL_REF_ATTR)) responseElement.setAttribute(ContentMapper.EL_REF_ATTR, this.pl.utils.uniqueNumber() + "");
            }
        };
    ...
    async renderChatUnits(cUnits: ChatUnit[]) {
        ...
        for (let i = cUnits.length - 1; i >= 0; i--) {
            ...
            while (true) {
                ...
                if (!chatUnitRef) {
                    this.pl.utils.prodWarn(`[ContentMapper] Chat unit has no ref attribute - 512`);
                    continue;
                }
        ...
        this.pl.utils.devLog(`[ContentMapper] Finished rendering ${cUnits.length} chat units.`);
    ...
    constructChatUnitElement(chatUnit: ChatUnit, promptCounter: number): HTMLElement {
        ...
        el.setAttribute(ContentMapper.EL_REF_ATTR, this.pl.utils.uniqueNumber().toString());
        ...
        if (!attrRefValue) this.pl.utils.prodWarn(`[ContentMapper] Failed to find attribute ${ContentMapper.EL_REF_ATTR} from real element - 4245`)
        ...
        el.addEventListener("click", async (e) => {
            ...
            if (sourceElement) {
                ...
                const dummyEl = this.pl.dummySpanElement;
                if (!dummyEl) this.pl.utils.prodWarn(`Missing dummy - k53`);
                else if (typeof dummyEl.focus !== "function") this.pl.utils.prodWarn("dummy not focusable - 7y4");
                else dummyEl.focus();
...
class GeminiAdapterChat {
    ...
    async init(): Promise<void> {
        ...
        await this.pl.utils.untilElementIdle(this.chatContainer, 3e3); // Wait for 3 seconds idle
    ...
    async getKeyElements() {
        this.chatContainer = await this.pl.utils.waitForAnElement(GeminiAdapterChat.CHAT_CONTAINER_SELECTOR);
    }
